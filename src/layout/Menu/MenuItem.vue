<template>
  <template v-if="!menu.hideMenu">
    ...
    <app-link class="el-menu-box" v-else-if="showMenuType === 1" :to="pathResolve">
      <el-menu-item :index="pathResolve" v-if="!menu.children[0].children || menu.children[0].children.length === 0">
        <i class="icon" :class="menu.children[0].meta.icon || menu.meta.icon"
           v-if="menu.children[0].meta.icon || menu.meta.icon"></i>
        <template #title>
          <span class="title">{{ menu.children[0].meta.title }}</span>
        </template>
      </el-menu-item>
